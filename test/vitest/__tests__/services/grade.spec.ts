...
describe('Grade', () => {
  ...
  describe('grade computations', () => {
    it('computeScaledGrade', () => {
      ...
      api.options = Object.assign({}, api.options, {
        options: {
          note_min: '1.0',
          note_max: '6.0'
        }
  ...
  describe('avg computation', () => {
    it('computes avg of a column in the array', () => {
      const gs = new GradeService();
      const result = gs.avg([{ a: 2.3 }, { a: 4.5 }, { b: 3.0, a: 4.6 }], (a) => a.a);
    ...
    it('avg returns count of non empty if non numeric', () => {
      const gs = new GradeService();
      const result = gs.avg(
        [{ a: 2 }, { a: null }, { a: '' }, { a: undefined }, { b: 3.0, a: 'demo' }],
        (a) => a.a
      );
  ...
  it('compute full example', () => {
    ...
    $http.get = vi.fn((url: string) => {
      if (url.includes('csv')) {
        return Promise.resolve({
          data: fs.readFileSync(path.resolve(__dirname, './students.in.csv'), 'utf8')
        });
      }
      if (url.includes('scores')) {
        return Promise.resolve({
          data: JSON.parse(fs.readFileSync(path.resolve(__dirname, './scores.json'), 'utf-8'))
        });
      }
      return Promise.resolve({});
    });
    ...
    return new Promise<void>((resolve) => {
      setTimeout(() => {
        ...
        expect($http.mock.calls[0][0].data).toBe(
          // normalize line endings to papaparse default
          fs.readFileSync(path.resolve(__dirname, './students.out.csv'), 'utf8').replace(/\r\n/g, '\n').replace(/\n/g, '\r\n')
        );
        resolve();
      }, 3000);
    });
