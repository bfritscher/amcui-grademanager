  ...
  <div class="fit column no-wrap">
    <div class="q-pa-md col-4 scroll">
      ...
      <p>
        ...
        <span v-if="scan.missing.incomplete > 0">Incomplete: {{ scan.missing.incomplete }}</span>
      </p>
      <template v-if="scan.missing.missing.length > 0">
        <h6 class="q-my-none">Missing pages: {{ scan.missing.missing.length }}</h6>
        <div>
          <div v-for="(missing, index) in scan.missing.missing" :key="index">
            {{ missing.student }}/{{ missing.page
            }}<span v-if="parseInt(String(missing.copy), 10) > 0">:{{ missing.copy }}</span>
          </div>
        </div>
      </template>
      <template v-if="scan.missing.failed.length > 0">
        <h6 class="q-my-none">Failed pages: {{ scan.missing.failed.length }}</h6>
        <div>
    ...
    >
      ...
      <template #header-cell-id="props">
        <q-th :props="props">
          {{ props.col.label }}
          <q-icon name="sym_o_help" size="1.5em">
            <q-tooltip>
          ...
          <q-icon name="sym_o_help" size="1.5em">
            <q-tooltip>
              <div style="max-width: 200px">
                The value MSD (mean square deviation) is an indication of the good framing of the
                marks (the four black dots surrounding each copy).<br />
      ...
      <template #header-cell-s="props">
        <q-th :props="props">
          ...
          <q-icon name="sym_o_help" size="1.5em">
            <q-tooltip>
              <div style="max-width: 200px">
                The value sensitivity is an indicator of proximity of the filling of the boxes with
                the threshold.<br />
                If it is too great (from 8 to its max value 10), you must check whether the boxes
                recognized as checked are the good ones.
              </div></q-tooltip
            >
          </q-icon>
        </q-th>
...
<script lang="ts">
import { defineComponent, reactive, onMounted, computed, ref, onUnmounted } from 'vue';
import formatDate from '../../utils/formatDate';
import { useRouter, useRoute } from 'vue-router';
import type { Page, PageScan } from '../models';
...
export default defineComponent({
  ...
  setup() {
    ...
    const scan = reactive({
      ...
      columns: [
        ...
        },
        {
          name: 'timestamp',
          label: 'Updated',
          field: (item: PageScan) => item.timestamp_manual || item.timestamp_auto,
          format: (val: string) => formatDate(parseInt(val, 10) * 1000, 'DD/MM/YYYY HH:mm'),
          sortable: true,
          align: 'left'
        },
        {
    ...
    function deleteScan(page: PageScan) {
      ...
      return API.$http
        .post(API.URL + '/project/' + route.params.project + '/capture/delete', page)
        .then(() => {
          scan.pages.splice(scan.pages.indexOf(page), 1);
          if (id === `${route.params.student}/${route.params.page}:${route.params.copy}`) {
            router.push({
              name: 'Scan',
              params: {
    ...
    function nextPage(step: number) {
      const currentPageIndex = scan.pages.findIndex((page) => page.id === selectedPage.value[0].id);
      let nextIndex;
      if (step > 0) {
    ...
    return {
      ...
      deleteAll() {
        $q.dialog({
          title: 'Warning!',
          message: 'This will remove all scans, grades and matchings. Do you want to continue?',
          cancel: true,
          ok: 'Delete All',
          color: 'negative',
          persistent: true,
