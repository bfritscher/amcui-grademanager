...
export default function examToHTML(localEditor: ExamEditor) {
  editor = localEditor;
  ...
  return Promise.all(html).then((html) => {
    ...
    a.download = editor.API.project + '.html';
...
function htmlQuizText(html: (string|Promise<string>)[], text: string) {
  ...
  text.replace(/<img id="(.*?)">/g, (subString, id:string) => {
    const p = fetch(editor.graphicsPreviewURL(id))
        .then((response:any) => {
  ...
  html.push(Promise.all(promises).then((results) => {
